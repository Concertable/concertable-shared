...
- First-ever review of a branch with no review markdown yet → use the **`code-review`** skill (it reviews the whole branch and creates the file + marker). Then use this one going forward.
...
Default: `reviews/<branch-slug>.md` (branch `/` → `-`). If the user named a file, or a legacy review file exists for this branch (e.g. `plans/PR_FEEDBACK.md`), use that. The file must already exist — this skill is incremental. If none exists, tell the user to run `/code-review` first.
...
## Step 3 — Run the `code-review` procedure over `start..HEAD`

Read `.claude/skills/code-review/SKILL.md` and **follow it from Step 3 onward verbatim**, with exactly one substitution: the review range is `<start>..HEAD` (the watermark SHA from Step 1/2), not the merge-base.
