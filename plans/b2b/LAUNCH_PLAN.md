...
- **DoorSplit/Versus revenue source** — these two settle against door/ticket revenue, which standalone B2B has no feed for. Money mechanic is identical for any non-owned-checkout option: Concertable charges the venue for the artist's share and settles it through Stripe (as FlatFee escrow already does); only *owned checkout* puts Concertable in the flow of the ticket funds directly. Options:
  ...
  - **(b) Import from a third-party ticketer** — *researched 2026-06-22 (two passes); ruled out for v1.* Buys only one thing over (a): a *verified* sales number instead of a self-reported one — the payout still funds by charging the venue. The merchant-of-record incumbents are unusable for it: only **DICE** exposes per-event sales (gated, read-only Partner API, no event creation), Skiddle's API is read-only + non-commercial-only + bars competing platforms, and Eventbrite's create/orders API is app-approval-gated — and all three hold the ticket money and pay the venue post-event. A second pass for the *self-serve, organiser-keeps-money* ticketers the first pass missed found that **Ticket Tailor** genuinely clears all three bars (public `POST event_series` + ticket-types, `ORDER.CREATED`/`ISSUED_TICKET` **webhooks**, and funds paid straight to the connected Stripe with no hold); TicketSource is a partial second (create-leg unverified). **But this doesn't rescue (b):** "organiser keeps the money" means the *venue's* Stripe, not Concertable's — so we'd get verified sales data but still settle by charging the venue. The only way Ticket Tailor gives us fund control is if **Concertable is the connected Stripe account**, which is just (c) wearing an external storefront. So (b) collapses to "verified data feed" — marginally better than (a), materially worse than (c) — and isn't worth the integration cost + per-provider lock-in at v1. Ticket Tailor is, however, the best post-launch *data-ingestion* path for venues that insist on their own ticketer.
...
- **2026-06-22 (follow-up pass)** — Second deep-research pass targeting the gap the first one left: *self-serve ticketers where the organiser keeps the money + a public API* (Ticket Tailor, TicketSource, Eventix/Weeztix, Billetto, Citizen Ticket, TryBooking, Universe, Fatsoma). Finding: **Ticket Tailor is the one platform that clears all three bars** — public event/ticket-type create API, `ORDER.CREATED`/`ISSUED_TICKET` webhooks for sales read-back, and funds paid straight to the connected Stripe with no platform hold (TicketSource partial second, create-leg unverified; Eventix has the API but marketplace-style held payouts). **This refines but does not reverse §9:** even Ticket Tailor routes funds to the *organiser's* Stripe, so option (A) only delivers fund control if Concertable *is* the connected Stripe account — which is option (c) with an external storefront. Net: own marketplace (B/c) is the only model that fully preserves the revenue-share USP at launch; (a) manual entry remains the v1 fallback; Ticket Tailor is a viable *post-launch data-ingestion* path, not a launch dependency. Billetto/Citizen Ticket/TryBooking/Universe/Fatsoma surfaced no citable findings (not reached, not ruled out). Sources: Ticket Tailor developer docs (create-event-series, webhooks, orders), TicketSource API/Stripe KB, DICE Partners GraphQL, Skiddle API terms, Weeztix docs + organiser terms, Eventbrite SEC 10-Q, GigPig FAQs.
