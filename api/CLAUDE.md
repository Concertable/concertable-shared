...
Anti-pattern, do not reintroduce: a tenant/owner key on the shared `ICurrentUser` (an `Owner` member + `GetOwnerId()` extension once lived in `Kernel` for exactly this, and were removed). Resolving *who owns a resource* is audience-specific and belongs to the consumer, not the shared identity contract — which carries only `Id` / `Email` / `IsAuthenticated`. As resolved: Payment reads the opaque `owner` claim at its own HTTP boundary (`Concertable.Payment.Api.Identity.ICurrentPayoutOwner`, fail-closed — throws if the claim is absent). **Customer** mints `owner` (the buyer's own id) and calls those endpoints directly. **B2B no longer mints `owner`**: its `Tenant.Api/StripeAccountController` fronts Payment's payout operations over gRPC, passing the active tenant id explicitly — sourced from B2B's request-scoped `ITenantContext` (a DB lookup off the user id), never the shared identity type.
